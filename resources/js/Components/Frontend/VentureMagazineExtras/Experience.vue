...
function trackExperienceClick(experience) {
    if (typeof window.gtag === 'function') {
        window.gtag('event', 'experience_click', {
            event_category: 'Experiences',
            event_label: experience.name,
...
<template>
    <div class="bg-white text-heavy">
        <section class="container mx-auto px-4">
            <h2 class="text-4xl font-bold text-left mb-12">Let’s Experience</h2>

            <SliderWrapper :items="experiences" :per-page="3" gap="1.5rem">
                <template #default="{ item }">
                    <div class="relative group overflow-hidden rounded-xl shadow-lg mb-10 bg-black/30 hover:bg-black/50 transition duration-500"
                        ...
                        <TileShare :url="`https://venturedownsouth.com.au/experience/${item.slug}`"
                            :title="item.name" :text="truncateWords(item.seo_description, 20)" />
                        ...
                        <div class="absolute inset-0 flex flex-col justify-center items-center text-white px-4 group">
                            ...
                            <div
                                ...
                                <a :href="`/experience/book/${item.slug}`" @click="trackExperienceClick(item)"
