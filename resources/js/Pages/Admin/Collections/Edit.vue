<template>
    <AdminLayout>
        ...
        <Head :title="`Edit Collection / Venture: ${form.name}`" />

        <div class="w-full mx-auto bg-white rounded-xl shadow-xl p-8">
            <div class="mb-6 border-b pb-4">
                <h2 class="text-2xl font-semibold text-blueGray-800">Edit Collection / Venture ({{ form.name }})</h2>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-6">
                    ...
                    <!-- collection Name -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Collection / Venture Name <span
                    ...
                    <!-- Embedded Type -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Type</label>
                        <Multiselect v-model="form.type" :options="['collection', 'venture']" :multiple="false"
                            :close-on-select="true" placeholder="Select Type" />
                    </div>

                    <!-- Embedded Content -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Content Listings</label>
                        <Multiselect v-model="form.content_ids" :options="contents" :multiple="true"
                            :close-on-select="false" track-by="id" label="title"
                            placeholder="Select Content Listings" />
                    </div>


                    <!-- Embedded Tours -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Select Tours</label>
                        <Multiselect v-model="form.tour_ids" :options="tours" :multiple="true" :close-on-select="false"
                            track-by="id" label="title" placeholder="Select Tours" />
                    </div>

                    <!-- Embedded Events -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Select Events</label>
                    ...
                    <!-- Embedded Towns -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Select Towns</label>
                    ...
                    <!-- Embedded Tags -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Select Tags</label>
                    ...
                    <div>
                        ...
                        <div v-if="collection.hero_image" class="mt-2">
                            <img :src="`/public/storage/${collection.hero_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-if="collection.big_hero_image" class="mt-2">
                            <img :src="`/public/storage/${collection.big_hero_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div v-if="collection.seo_image" class="mt-2">
                            <img :src="`/public/storage/${collection.seo_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
...
const editorContent = ref(JSON.parse(props.collection.description))
...
const form = useForm({
    ...
    seo_title: props.collection.seo_title,
    seo_description: props.collection.seo_description,
    seo_image: null,
    type: props.collection.type,
    content_ids: props.collection.items || [],
    tour_ids: props.collection.tours || [],
    event_ids: props.collection.events || [],
    tag_ids: props.collection.tags || [],
    town_ids: props.collection.towns || [],
})
...
function submit() {
    ...
    form.post(route('admin.collections.update', props.collection.id), {
        preserveScroll: true,
        forceFormData: true,
