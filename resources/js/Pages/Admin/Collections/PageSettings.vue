<template>
    <AdminLayout>
        ...
        <Head :title="`Edit Collection Page Settings`" />

        <div class="w-full mx-auto bg-white rounded-xl shadow-xl p-8">
            <div class="mb-6 border-b pb-4">
                <h2 class="text-2xl font-semibold text-blueGray-800">Edit Collection Page Settings</h2>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-6 mb-10">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Hero Image (Minimum image size:
                            738×500px (width × height) <span
                            class="text-red-500">*</span></label>
                        <input type="file" @change="handleHero" />
                        <p v-if="form.errors.hero_image" class="text-red-500 text-sm mt-1">{{ form.errors.hero_image }}
                        </p>
                        <div v-if="setting.hero_image" class="mt-2">
                            <img :src="`/public/storage/${setting.hero_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-if="setting.big_hero_image" class="mt-2">
                            <img :src="`/public/storage/${setting.big_hero_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div v-if="setting.seo_image" class="mt-2">
                            <img :src="`/public/storage/${setting.seo_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                </div>
                <!-- Actions -->
...
const editorContent = ref(JSON.parse(props.setting.description))
...
function submit() {
    ...
    form.post(route('admin.collections.settings.update', props.setting.id), {
        preserveScroll: true,
        forceFormData: true,
