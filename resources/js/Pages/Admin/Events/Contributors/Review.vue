...
const renderedOpening = computed(() => {
    if (!props.event.opening_times) return ''
    try {
        const json = typeof props.event.opening_times === 'string'
            ? JSON.parse(props.event.opening_times)
            : props.event.opening_times
...
const tabs = computed(() => {
    const availableTabs = []
    if (props.event.video) availableTabs.push('video')
    if (props.event.location) availableTabs.push('map')
    if (props.event.custom_fields) availableTabs.push('information')
...
const cleanVideoIframe = computed(() => {
    if (!props.event.video) return null

    return props.event.video
        .replace(/width="\d+"/, 'width="100%"')
        .replace(/height="\d+"/, 'height="500px"')
})
const googleMapsUrl = computed(() => {
    ...
    return baseUrl + encodeURIComponent(props.event.address)
})
...
const formattedDate = computed(() => {
    const { start_date, end_date, event_date_label } = props.event || {}
    ...
    if (start_date && event_date_label) {
        return `${format(start_date)} · ${event_date_label}`
    }
...
const shareUrl = computed(() =>
    shareAbs(`/event/${props.event.slug}`)
)
...
const twitterUrl = computed(() =>
    `https://twitter.com/intent/tweet?url=${enc(shareUrl.value)}&text=${enc(props.event.name)}`
)
...
const nativeShare = async () => {
    try {
        await navigator.share({
            ...
            text: (props.event.summary || '').slice(0, 140),
            url: shareUrl.value
        })
    } catch { }
}
...
// Convenience flags
const canModerate = computed(() => props.event.status === 'pending') // show actions only when pending
...
function approve() {
    router.post(
        route('admin.events.approve', props.event.id),
...
function reject() {
    ...
    router.post(
        route('admin.events.reject', props.event.id),
            ...
            <div
                ...
                <div class="space-y-6 md:col-span-3">
                    ...
                    <div v-if="event.tags?.length">
                        ...
                        <Link v-for="tag in event.tags" :key="tag.id" :href="`#`"
                    ...
                    <!-- Towns -->
                    <div v-if="event.towns?.length">
                        <h3 class="text-xl font-semibold text-bison mb-2 pb-3 w-5/6 border-b border-envy">Towns</h3>
                        <Link v-for="town in event.towns" :key="town.id" :href="`/town/${town.slug}`"
                            class="px-3 py-1 tracking-wide uppercase rounded-full bg-gray-100 text-heavy text-lg block font-semibold hover:text-center hover:bg-heavy hover:text-white transition">
                        {{ town.name }}
                        </Link>
                    </div>
                ...
                <div class="space-y-6 md:col-span-3">
                    <div class="lg:col-span-2">
                        <p class="text-lg font-semibold text-heavy text-center mb-5 tracking-wider hover:text-bison cursor-pointer"
                            data-aos="fade-left">
                            {{ formattedDate }}
                        </p>
                        <div
                            ...
                            <!-- Buttons -->
                            <div v-if="event.custom_buttons?.length" class="flex flex-wrap gap-3 justify-center">
                                <a v-for="(field, i) in event.custom_buttons" :key="i" :href="field.value"
                            ...
                            <!-- Address Line -->
                            <div class="flex items-start space-x-2" v-if="googleMapsUrl && event.address">
                            ...
                            <!-- Opening Time -->
                            <div class="flex items-start space-x-2 cursor-pointer" v-if="event.opening_times">
                            ...
                            <!-- Phone Numbers -->
                            <div v-if="event.phone_number" class="flex items-center space-x-2">
                                <PhoneIcon class="w-5 h-5 text-bison" />
                                <div class="flex flex-wrap gap-3">
                                    <a v-for="(phone, index) in event.phone_number.split(',')" :key="index"
                                        :href="`tel:${phone.trim()}`"
                                        class="text-bison hover:text-white font-semibold  whitespace-normal">
                                        {{ phone.trim() }}
                                    </a>
                            ...
                            <!-- Mail -->
                            <div v-if="event.email" class="flex items-center space-x-2">
                                <Mail class="w-5 h-5 text-bison" />
                                <div class="flex flex-wrap gap-3">
                                    <a v-for="(email, index) in event.email.split(',')" :key="index"
                            ...
                            <div v-if="event.social_links?.length" class="flex flex-wrap gap-3 justify-center">
                                <a v-for="(field, i) in event.social_links" :key="i" :href="normalizeUrl(field.value)"
                                    class="text-sm font-semibold text-white border border-white/30 px-4 py-2 rounded-full hover:border-bison hover:text-bison transition-all"
                                    target="_blank" rel="noopener noreferrer">
