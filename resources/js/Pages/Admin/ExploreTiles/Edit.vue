<template>
    <AdminLayout>
        ...
        <Head title="Edit Image Tile" />

        <div class="w-full mx-auto p-6 bg-white rounded-xl shadow-xl">
            <div class="mb-6 border-b pb-4">
                <h2 class="text-2xl font-semibold text-blueGray-800">Edit Image Tile ({{ form.title }})</h2>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div v-if="tile.image" class="mt-2">
                            <img :src="`/public/storage/${tile.image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
...
const form = useForm({
    ...
    subtitle: props.tile.subtitle,
    image: null,
    link: props.tile.link,
    is_active: true,
    source: props.tile.source,
    sort_order: props.tile.sort_order,
})
...
function submit() {
    form.post(route('admin.explore-tiles.update', props.tile.id), {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => toast.success('Image Tile updated successfully!'),
        onError: () => toast.error('Failed to update image tile'),
    })
}
</script>