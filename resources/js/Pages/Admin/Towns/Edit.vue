<template>
    <AdminLayout>
        ...
        <Head :title="`Edit Town: ${form.name}`" />

        <div class="w-full mx-auto bg-white rounded-xl shadow-xl p-8">
            <div class="mb-6 border-b pb-4">
                <h2 class="text-2xl font-semibold text-blueGray-800">Edit Town ({{ form.name }})</h2>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-6">
                    ...
                    <!-- Town Name -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Town Name <span
                    ...
                    <div>
                        ...
                        <div v-if="town.hero_image" class="mt-2">
                            <img :src="`/public/storage/${town.hero_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-if="town.big_hero_image" class="mt-2">
                            <img :src="`/public/storage/${town.big_hero_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>

                    <!-- Rezdy Category URL -->
                    <div>
                        <label class="block text-sm font-bold text-blueGray-700 mb-1">Rezdy URL (optional)</label>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div v-if="town.seo_image" class="mt-2">
                            <img :src="`/public/storage/${town.seo_image}`" class="h-24 rounded shadow" />
                        </div>
                    </div>
                ...
                <div class="col-span-1 md:col-span-2 flex justify-end space-x-3 pt-4 border-t">
                    <Link :href="route('admin.towns.index')"
...
const editorContent = ref(JSON.parse(props.town.description))
const form = useForm({
    _method: 'PUT',
    name: props.town.name,
    rezdy_url: props.town.rezdy_url,
    hero_image: null,
    big_hero_image: null,
    description: props.town.description,
    summary: props.town.summary,
    is_active: Boolean(props.town.is_active),
    seo_title: props.town.seo_title,
    seo_description: props.town.seo_description,
    tag_ids: props.town.tags || [],
    seo_image: null,
})
...
function submit() {
    ...
    form.post(route('admin.towns.update', props.town.id), {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => toast.success('Town updated successfully!'),
        onError: () => toast.error('Failed to update town'),
    })
}
</script>
