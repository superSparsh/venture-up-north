<template>
    <ContributorLayout>
        ...
        <section class="pb-10">
            <div class="w-full mx-auto p-6 bg-white rounded-xl shadow-xl mt-10 mb-5">
                <div class="mb-6 border-b pb-4">
                    <h2 class="text-2xl font-semibold text-blueGray-800">Add New Event</h2>
                </div>
                <div class="mb-6">
                ...
                <form @submit.prevent="submit">
                    <div class="space-y-6">
                        <div v-show="currentTab === 'Basic Info'" class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
                            <div class="space-y-6">
                                ...
                                <!-- Embedded Town -->
                                <div>
                                    <label class="block text-sm font-bold text-blueGray-700 mb-1">Towns <span
                                            class="text-red-600">*</span></label>
                                    <Multiselect v-model="form.town_ids" :options="towns" :multiple="true"
                                        :close-on-select="false" track-by="id" label="name"
                                ...
                                <div>
                                    ...
                                    <textarea v-model="form.summary" rows="3"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-200 focus:outline-none" />
                                    <p v-if="form.errors.summary" class="text-red-500 text-sm mt-1">{{
                                        form.errors.summary
                                        }}</p>
                                </div>
                                <!-- Embedded Tags -->
                        ...
                        <div v-show="currentTab === 'Event Info'" class="space-y-6 mb-10">
                            <div>
                                ...
                                <input type="text" v-model="form.event_date_label"
                                    placeholder="e.g. 1st Friday each month" class="form-input w-full" />
                            ...
                            <div>
                                ...
                                <Datepicker v-model="form.start_date" :enable-time-picker="false" :min-date="today"
                                    placeholder="Select start date" :format="dateFormat" />
                        ...
                        <div v-show="currentTab === 'Media'" class="space-y-6 mb-10">
                            ...
                            <div>
                                ...
                                <p v-if="form.errors.hero_image" class="text-red-500 text-sm mt-1">{{
                            ...
                            <!-- Big Hero Image -->
                            <div>
                                <label class="block text-sm font-bold text-blueGray-700 mb-1">Big Hero Image (Minimum
                                    image
                        ...
                        <div v-show="currentTab === 'Location & Embed'" class="space-y-6 mb-10">
                            ...
                            <div>
                                ...
                                <input v-model="form.video" type="text"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-200 focus:outline-none" />
                                <p v-if="form.errors.video" class="text-red-500 text-sm mt-1">{{ form.errors.video }}
                                </p>
                            </div>
                        </div>

                        <div v-show="currentTab === 'Contact Info'"
                            class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
                            <div class="space-y-6">
                                <!-- Address -->
                                <div>
                        ...
                        <div v-show="currentTab === 'Ticket Link'" class="space-y-6 mb-10">
                            ...
                            <draggable v-model="form.custom_buttons" :item-key="index" handle=".handle"
                                class="space-y-2">
                        ...
                        <div>
                            ...
                            <!-- SEO Image -->
                            <div>
                                <label class="block text-sm font-bold text-blueGray-700 mb-1">Social Share Image
                                    (Minimum image size:
                                    1200×600px (width × height))</label>
                                <input type="file" @change="handleSeo" />
                                <p v-if="form.errors.seo_image" class="text-red-500 text-sm mt-1">{{
                    ...
                    <div
                        class="col-span-1 md:col-span-2 flex justify-end space-x-3 pt-4 border-t-2 border-bison border-dashed">
                        <TermsAgreement :terms="props.terms" v-model="form.agreements" class="mb-6 mt-5" />
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2 flex justify-end space-x-3 pt-4 border-t">
                        <!-- Cancel -->
                        <Link :href="route('contributor.events.index')"
                            class="text-sm px-5 py-2 border rounded-md text-gray-600 hover:bg-gray-100">
                        Cancel
                        </Link>

                        <!-- Save as Draft -->
                        <button type="button" @click="submitEvent('draft')" :disabled="form.processing"
                            class="bg-cyan-500 text-white hover:bg-cyan-700 font-semibold px-6 py-2 rounded-md shadow">
                            Save as Draft
                        </button>

                        <!-- Submit for Review -->
                        <button type="button" @click="submitEvent('pending')" :disabled="form.processing"
...
async function submitEvent(status) {
    ...
    form.post(route('contributor.events.store'), {
        preserveScroll: true,
        forceFormData: true,
        onSuccess: () => {
