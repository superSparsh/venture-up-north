...
function submit(e) {
    ...
    router.get(route('ventures.community'), filters, {
        preserveScroll: true,
        preserveState: true,
        replace: true
    })
        ...
        <section class="relative w-full h-screen overflow-hidden text-white">
            ...
            <div class="relative z-20 h-full flex items-center" data-aos="fade-up">
                <div class="container mx-auto px-4">
                    ...
                    <p class="mt-3 max-w-2xl text-white/80">Browse ventures created by travellers like you.</p>
                </div>
            </div>
        </section>

        <section class="container mx-auto px-4 py-8 text-heavy space-y-6 mb-5">
            <VentureTabs active="community" />
            <!-- Filters -->
            <form class="bg-heavy border border-bison rounded-2xl p-4 grid gap-3 md:grid-cols-4"
                ...
                <div class=" flex justify-end gap-2">
                    ...
                    <Link :href="route('ventures.community')"
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <VentureCard v-for="v in ventures.data" :key="v.id" :venture="v"  :to="`/ventures/${v.slug}`" />
            </div>
